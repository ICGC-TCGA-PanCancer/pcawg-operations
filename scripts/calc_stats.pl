...
# d <- read.csv(file="in_whitelist.tsv", sep="\t", head=TRUE)
# e <- read.csv(file="out_whitelist.tsv", sep="\t", head=TRUE)
# perl calc_stats.pl donor_p_150318030103.jsonl master.whitelist.txt in_whitelist.tsv out_whitelist.tsv

my ($file, $whitelist, $in_list_output, $out_list_output) = @ARGV;

my $json = JSON->new->allow_nonref;

open OUTIN, ">$in_list_output" or die;
open OUTOUT, ">$out_list_output" or die;
